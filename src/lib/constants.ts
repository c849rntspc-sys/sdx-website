...
export const PLATFORM_FEATURES = [
  ...
  { icon: 'Layers', title: 'Fund organization', description: 'Organize properties into investment funds (Core, Value-Add, Opportunistic). Filter portfolio and benchmarks by fund. Fund-level analytics in Data Explorer with weighted-average performance metrics. REITs see a flat portfolio; investment managers see fund groupings.' },
  ...
  { icon: 'Target', title: 'Goal & pathway tracking', description: 'Set decarbonization targets aligned to CRREM 1.5°C pathways, SBTi Buildings Sector, LL97/BEPS/MEES regulatory thresholds, GRESB score improvement, or custom internal targets. Framework goal cards show real-time progress with on-track/at-risk/off-track status badges. Goals cascade from entity to fund to building level.' },
  { icon: 'UserPlus', title: 'Tenant & PM portal', description: 'Mobile-first PWA with data quality dashboard, action items with severity badges, property cards with meter health indicators, and direct data contribution — readings, bill uploads, and meter submissions. Installable on mobile home screens.' },
  { icon: 'UserCheck', title: 'Building-level delegation', description: 'Invite tenants and property managers to specific buildings with read-only or edit access. Track invitation status, assigned properties, and last login from Settings and Building Profile pages.' },
  { icon: 'Languages', title: '14-language internationalization', description: 'Full UI translation across English, Spanish, French, German, Japanese, Chinese, Korean, Portuguese, Italian, Arabic, Hindi, Swedish, Norwegian, and Danish. Language selector on login for immediate localization.' },
  { icon: 'RefreshCw', title: 'Dynamic units & currency', description: 'User preferences for unit system (metric/imperial) and currency applied globally across all pages, charts, and reports. Automatic conversion (m²↔ft², kWh/m²↔kBTU/ft², kg↔lb). 15 currencies supported.' },
  { icon: 'PanelLeftClose', title: 'Collapsible sidebar', description: 'Clean, space-efficient navigation with a vertical toggle. Collapsed state shows icon-only nav with logout shortcut — maximizing workspace for data-heavy views.' },
